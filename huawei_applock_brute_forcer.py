...
def bruteforcer(params_dict, attack_type, wordlist=None):
    count = 0
    if attack_type == 'pin':
        print('Bruteforcing PIN. 0-9999 (10000 candidates)...')
        while count < 10000:  # total number of PIN candidates is 9999
            _hash = derive_hash(f'{count:04}', params_dict['salt'])  # pad count to make 4 digit PIN
            ...
            if hexlify(derived_key).decode()[:24] == params_dict['match']:
                return ['Cracked',
                        'PIN: {}'.format(count),
                        'Derived Key: {}'.format(hexlify(derived_key).decode())]  # return PIN and derived key
            progress(round((count / 9999) * 100))
            count += 1
        return False
    else:
...
def fetch_params(input_file):
    ...
    for row in cursor.fetchall():
        ...
        if 'encrypt_password_sha256_salt' in row:
            params_dict['salt'] = row[2]
        if 'encrypt_password_sha256' in row:
            args_ = row[2].split(':')
